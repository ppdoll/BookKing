...
import { requireUser, getMemberships, isClassroomStudent } from "@/lib/session";
...
export default async function WrappedPage({
  ...
  return (
    <div style={{ maxWidth: 520, margin: "0 auto" }}>
      ...
      <WrappedCard stats={stats} hideCommercial={isStudent} />

      {isStudent ? (
        <section className="card" style={{ marginTop: 16 }}>
          <h3 style={{ margin: "0 0 6px", fontSize: 15 }}>🌐 전체 공개</h3>
          <p className="mini" style={{ margin: 0 }}>
            🏫 학교 모드에서는 개인정보 보호를 위해 결산을 인터넷에 공개할 수 없어요. 우리 반 안에서만 볼 수 있어요.
          </p>
        </section>
      ) : (
        <section className="card" style={{ marginTop: 16 }}>
